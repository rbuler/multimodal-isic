...
def _sort_group_patches(g):
        ...
        if 'patch_id' in g.columns:
            try:
                return g.sort_values('patch_id').reset_index(drop=True)
            except Exception:
                return g
        ...
        if g['_patch_num'].notnull().all():
            return g.sort_values('_patch_num').reset_index(drop=True)
...
models = [
    "a9d7feb3402a4670bbcfa73f534acab7.pth",  # 799
    "ce4069521dfb4264a3ac8cc3d59971a2.pth",  # 726
    "e6b29aa3b47145ec935e675a13c4b71d.pth",  # 804
    "c72210e208974529927e6c53d8ec890c.pth",  # 805
    "4175dac48c3b4e93b4c0c82e8d8b44ff.pth",  # 806
    "6d4c4f1198f0439583ffd3af0a76ef9f.pth",  # 802
]
...
for model_name in models:
    ...
    for fold_idx, (train_idx, val_idx) in enumerate(skf.split(np.zeros(len(y)), y)):
        print(f"  Model {model_name} | Fold {fold_idx+1}/{SPLITS}: train {len(train_idx)} patients, val {len(val_idx)} patients")
