...
train_transform = A.Compose([A.Resize(380,380), A.HorizontalFlip(p=0.5), A.VerticalFlip(p=0.5),
...
valid_transform = A.Compose([A.Resize(380,380),
                             A.Normalize(mean=(0.485,0.456,0.406), std=(0.229,0.224,0.225)), ToTensorV2()])
...
for epoch in range(num_epochs):
    ...
    if config["neptune"]:
        ...
        img = batch['image'][0:1].to(device)  # shape: (1, C, H, W)
        ae_model.eval()
        with torch.no_grad():
            recon, _ = ae_model(img)
        ...
        buf.seek(0)
        run["visuals/image_comparison"].append(neptune.types.File.from_content(buf.getvalue(), extension='png'))
        plt.close(fig)
