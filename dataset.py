...
class DermDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        age = torch.tensor(row['age_normalized'], dtype=torch.float) if 'age_normalized' in row.index else torch.tensor(0.0, dtype=torch.float)
        sex = torch.tensor(row['sex_encoded'], dtype=torch.long) if 'sex_encoded' in row.index else torch.tensor(0, dtype=torch.long)
        loc = torch.tensor(row['loc_encoded'], dtype=torch.long) if 'loc_encoded' in row.index else torch.tensor(0, dtype=torch.long)
        artifacts = torch.tensor(row[self.artifact_cols].values.astype(int), dtype=torch.long) if all(col in row.index for col in self.artifact_cols) else torch.zeros(len(self.artifact_cols), dtype=torch.long)
